...
class CampaignController extends Controller
{
    ...
    public function store(Request $request, \App\Models\Project $project)
    {
        ...
        \App\Jobs\GenerateCampaignJob::dispatch($campaign->id);
    ...
    public function approve(Request $request, Campaign $campaign)
    {
        ...
        try {
            ...
            $hasActiveCampaign = \App\Models\Campaign::where('project_id', $campaign->project_id)
        ...
        try {
            ...
            if ($creditDeducted && !$approved) {
                try {
                    ...
                    \Illuminate\Support\Facades\Log::critical('Failed to auto-refund credit after approval failure', [
                        'user_id' => $user->id,
        ...
        if ($approvalException instanceof \Illuminate\Contracts\Cache\LockTimeoutException) {
            return redirect()->route('projects.show', $campaign->project_id)->with('error', 'System is busy processing another request. Please try again.');
        }
        ...
        if (in_array($campaign->status, ['active', 'paused'])) {
            ...
            if ($publishedCount === 0) {
                try {
                    ...
                    \Illuminate\Support\Facades\Log::error('Failed to refund credit on campaign destroy', ['error' => $e->getMessage()]);
                }
            }
        }
