...
class ProcessDodoWebhookJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if (empty($webhookSecret)) {
            \Illuminate\Support\Facades\Log::critical('DODO_WEBHOOK_SECRET is not configured. Webhook processing is INSECURE — any caller can trigger credit grants and refunds.', [
        ...
        if (!empty($storedSignature)) {
            ...
            if (!hash_equals($recomputed, (string) $storedSignature)) {
                \Illuminate\Support\Facades\Log::critical('Dodo webhook signature mismatch in queued job — possible tampering or replay.', [
                    'webhook_log_id' => $this->webhookLog->id,
                    'event_type' => $eventType,
                ]);
            ...
            \Illuminate\Support\Facades\Log::info('Dodo webhook signature not stored on WebhookLog — defense-in-depth signature check skipped.', [
                'webhook_log_id' => $this->webhookLog->id,
            ]);
        }
        ...
        try {
            ...
            DB::transaction(function () use ($payload, $eventType) {
                ...
                if (!$customerEmail) {
                    \Illuminate\Support\Facades\Log::warning("Received {$eventType} webhook with no customer email; skipping credit/refund logic.", [
                        'event_id' => $eventId,
                    ]);
                } else {
                    ...
                    if (!$user) {
                        \Illuminate\Support\Facades\Log::warning("Received {$eventType} webhook for unknown email: {$customerEmail}", [
                            'event_id' => $eventId,
                        ]);
                    } else {
                        ...
                        if ($eventType === 'payment.failed') {
                            ...
                            \Illuminate\Support\Facades\Log::info("Dodo payment.failed received for user {$user->id}; no credits granted.", [
                                'event_id' => $eventId,
                                'amount' => $amount,
                            ]);
                        }
                        ...
                        if ($eventType === 'payment.succeeded') {
                            ...
                            if ($productId) {
                                if ($productId === config('services.dodo.link_1_campaign')) $creditsToAdd = 1;
                                if ($productId === config('services.dodo.link_3_campaigns')) $creditsToAdd = 3;
                                if ($productId === config('services.dodo.link_10_campaigns')) $creditsToAdd = 10;
                            }
                            ...
                            if ($creditsToAdd > 0) {
                                ...
                                \Illuminate\Support\Facades\Log::warning("Received payment.succeeded for user {$user->id} with unmapped amount: {$amount}", [
                                    'event_id' => $eventId,
                                ]);
                            }
                        }
                        ...
                        if ($eventType === 'payment.refunded') {
                            ...
                            $originalTransaction = \App\Models\CreditTransaction::where('user_id', $user->id)
                                ->where('type', 'purchase')
                            ...
                            if ($originalTransaction) {
                                ...
                                if ($fallbackCredits > 0) {
                                    \Illuminate\Support\Facades\Log::info("Refund: original transaction not found, using tier-based fallback of {$fallbackCredits} credits.", [
                                        'user_id' => $user->id,
                                    ...
                                    \Illuminate\Support\Facades\Log::warning("Refund: no matching transaction and unmapped refund amount for user {$user->id}.", [
                                        'refund_amount' => $refundAmount,
                                        'event_id' => $eventId,
                                    ]);
                                }
